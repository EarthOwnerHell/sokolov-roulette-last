...
const randomDependingMode = {
  wheel: function () {
    ...
    const numberType =
      number % 2 == 0 && number != 0 ? 'even' : number % 2 >= 1 && number != 0 ? 'odd' : '';
    const interval = range(1, 12).includes(number)
      ? '1-12'
      : range(13, 24).includes(number)
      ? '13-24'
      : range(25, 36).includes(number)
      ? '25-36'
      : 0;
  ...
  dice: function () {
    ...
    const numberType = number % 2 == 0 ? 'even' : 'odd';
    const interval = range(1, 4).includes(number)
      ? '1-4'
      : range(5, 8).includes(number)
      ? '5-8'
      : range(9, 12).includes(number)
      ? '9-12'
      : 0;
    return number == 0
      ? [number, color, interval, numberType, 'Золото']
      : [number, color, interval, numberType];
  },
};
