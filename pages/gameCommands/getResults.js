...
const {
  ...
  editWinToday,
  editLossToday,
} = require('../../database/managers/global');
const {
  plusBalanceUser,
  editWinPerDay,
  editWinPerWeek,
  plusWinCubesAll,
} = require('../../database/managers/user');
...
const getWinnersAndLoosers = async (data) => {
  ...
  const winCoombination = `${results.length != 5 ? results[0] : results[4]} ${
    gamePayloadsTranslate[results.length == 1 ? results[0] : results[1]][0]
  }`;

  let textToReturn = `🎰 ${
    Number(results[0]) || (results[0] == 0 && results.length < 5)
      ? `Выпало число ${winCoombination}`
      : results[0] == 0 && results.length == 5
      ? `Выпало ${winCoombination}`
      : `Выпал коэффицент ${winCoombination}`
  }\n\n`;
  ...
  for await (const userBet of bets) {
    ...
    const fullBet = userBetAmount * gamePayloadsTranslate[userBetType][3];
  ...
  vkHelp({
    peer_id: 2000000263,
    message: `${textToReturn}\n\nРежим игры: ${gameMode}\nВ чате: ${chatLink}\n\nИтог: ${numberWithSpace(
      statsForAdm.toFixed(0),
    )} кубиков`,
  });
...
function checkResults() {
  setTimeout(async function () {
    ...
    for (let i = 0; i < games.length; i++) {
      ...
      if (Date.now() - thisGame?.endTime >= -1_000 && Date.now() - thisGame?.endTime <= 0)
        vkHelp({ peer_id: thisGame.peerId, message: '🔮 Раунд подходит к концу...' });
      ...
      if (thisGame?.endTime && Date.now() - thisGame?.endTime >= 0) {
        ...
        const deductionsToTop = `\n📊 Отчисления в топы: ${numberWithSpace(
          finalText[1].toFixed(0),
        )}`;
        ...
        vkHelp({
          ...
          attachment: photoesDependMode[gameMode][results[0]]
            ? photoesDependMode[gameMode][results[0]]
            : photoesDependMode[gameMode][results[1]],
        });
      }
    }
