...
module.exports = profile = async (msg) =>  {
    const { name, balance, bonuseBalance, id, ref:{ value } } = await getUser(msg.userId)
    ...
    messageEdit({peer_id: peerId, message_id: lastMessage, message: 
`👤 @id${id}(${name}), твой профиль:\n\n🔗 Рефералов у вас: ${numberWithSpace(value)}\n———\n\n💰 Баланс: ${numberWithSpace(balance.toFixed(0))} 🎲\n———\n\n🎁 Бонусный баланс: ${numberWithSpace(bonuseBalance.toFixed(0))} 🎲`, keyboard: profileBoardWithCallback})
}
